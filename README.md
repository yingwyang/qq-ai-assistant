...
| 月卡订阅 | 小月卡/大月卡/ALL 全功能版，支持退款与纠纷处理；月卡每日额外积分（小 +100 / 大 +300）随**每日签到**发放，**双持叠加 +400** |
| 管理后台 | 用户管理、订单审批、积分调账、组件控制（已拆分为 13 个懒加载子页）；**积分规则配置**为分节导航 + 结构化行编辑器（模型费率/分析倍率/阶梯折扣不再手写 JSON）+ 未保存脏标记 + 内联校验 + **费用试算器** |
