...
| 资金流水（后台） | 在积分流水上叠一层**模拟现金账**：订阅按套餐价计收入、退款按比例计支出、AI/TTS 按积分成本折算；含 KPI 卡、现金收支趋势/构成/类型分布三张图、现金列与类别标签，管理员可**手工记一笔**（`POST /api/credits/admin/cash`，不改积分）。现金不写库、读取时推算，改规则即改口径。导出支持 **CSV**（与表格同列，`format=csv`，上限 5 万行）与 JSON |
...
| 2026-09-20 | 适配 AstrBot 4.28.1：模型列表改从 `cmd_config.json` 读取、并以 `config/abconf_*.json` 档案兜底（4.28 已移除 `/api/v1/models`，实测 404）；对话模型选择补上 `selected_provider`/`selected_model`（4.28 只认这两个字段，此前传 `model` 等于选择被静默忽略）并做模型存在性校验，避免无效 provider 让 AstrBot 直接报错；AI 对话设置里的「已配置的模型」改为展示 AstrBot 真实配置（此前只有在本对话框手填提供商 Key 后才有值，导致 AstrBot 里 27 个模型却显示"暂无模型"）；修复新增提供商表单中「确定 / 取消」被 flex 压到 36px、中文竖排换行的按钮 bug |
| 2026-09-24 | 修复「普通用户无法登录 QQ」：机器人登录二维码三入口（`/api/system/napcat/{qrcode,qrcode-path,qrcode-image}`）此前被批次 A 收成 `ROLE_ADMIN`，普通用户 403 导致扫码登录不可用；现放宽为**登录即可读**（`authenticated()`，匿名仍 401），前端用户菜单与 `LoginModal` 的二维码区对所有登录用户开放；组件启停仍保持管理员专属 |
