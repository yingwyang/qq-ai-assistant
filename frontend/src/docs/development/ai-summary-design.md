...
>   群白名单（生成与读取都受约束）、聊天页「历史速览」日期切换 + 「全部历史」分页列表（每页 30、最多 200）
>   + 「导出 Markdown」（纯前端 Blob 下载，不新增后端接口）。
> - 仍未做（按优先级）：摘要标签点击跳转搜索（设计稿 §前端交互 1 提到，未实现）。
> - **日报推送到 QQ 群：仅手动触发**（速览卡片上的「推送到群」按钮 → 展示推送原文二次确认 → `POST /api/groups/{id}/digest/push`，仅管理员）。
>   ⚠️ 明确约束：**不要接定时任务自动推送**，`digest-cron` 只负责「生成日报」，推送必须由人点击确认。
> - **摘要标签可点击搜索**：点击速览卡片或消息摘要里的标签 → 右侧搜索面板按 `ai_tags` 检索（可叠加关键词），
>   点击结果会滚动并高亮该消息（不在当前加载范围时给出提示）；对应接口 `GET /api/messages/search?groupId=&tag=&keyword=&limit=`。
