...
> **状态：设计稿，尚未实装。** 本文只定义方案（接口 / 表结构 / 交互 / 风控参数），代码未做改动。
...
| **阶段 1** | 单条按需摘要（接口 + 按钮 + 限流 + 幂等） | 约 1 天 | 点击按钮 2–16 秒内出卡片；重复点击返回缓存不重复扣费；无权限消息返回 403 |
| **阶段 2** | 批量工具（参数化 + 预估 + 进度 + 停止 + 审计） | 约 1–2 天 | 不带范围提交返回 400；投递 200 条能实时看到进度；点停止后队列归零 |
| **阶段 3** | 群日报（新表 + 服务 + 独立队列 + 前端速览 + 可选定时） | 约 1 天 | 同群同天重复调用返回缓存；20 个群 6 分钟内跑完；群聊页可见今日速览 |
